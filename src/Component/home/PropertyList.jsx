...
const Card = ({id, image, address, price, name }) => {
    const handleImageError = (e) => {
        e.target.src = '/assets/image1.jpeg'; // Fallback to a local image
    };

    return (
        <Link to={`/propertylist/${id}`} className='property-card-link'>
            <figure className='property'>
                <img src={image} alt='Propertyimg' onError={handleImageError} />
                <h4>{name}</h4>
                <figcaption>
